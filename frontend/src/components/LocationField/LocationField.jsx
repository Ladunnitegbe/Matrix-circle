...
export default function LocationField({ label = 'Set Location', value, onLocate, error = false, caption1, required = false, className = '' }) {
  ...
  return (
    <Input
      label={label} required={required} readOnly onClick={handleLocate}
      placeholder={locating ? 'Locating…' : 'use current location'}
      value={displayValue} error={error || Boolean(geoError)} caption1={geoError || caption1} className={className}
      trailingAction={
        <button type="button" onClick={handleLocate} disabled={locating} aria-label={value ? 'Update location' : 'Use current location'}
