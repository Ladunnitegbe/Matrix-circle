...
const TONE_ICON = { info: AlertCircleIcon, success: CheckIcon, warning: AlertIcon, error: AlertIcon };
const TONE_STYLES = {
...
export default function Alert({ tone = 'info', title, children, onDismiss, className = '' }) {
  ...
  return (
    <div role={tone === 'error' ? 'alert' : 'status'} className={['flex items-start gap-3 rounded-lg border px-4 py-3', toneClasses, className].filter(Boolean).join(' ')}>
      ...
      {onDismiss && (
        <button type="button" onClick={onDismiss} aria-label="Dismiss" className="flex-shrink-0 rounded focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent-orange">
